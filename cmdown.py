...
def download_callback(downloader):
    ...
    if downloader.get_state() == Downloader.DOWNLOADING:
        ...
        if len(written_update) < term_width * 3 / 4:
            ...
            written_update += ' ╢{}{}╟   Size: {}'.format(done, remaining, down.readable_bytes(downloader.total_length))
        else:
        ...
        written_update = fg + "\t[Merging {:5}/{:5}]".format(down.readable_bytes(downloader.total_merged),
...
def run(argv):
    ...
    parser = argparse.ArgumentParser(description='CMdown - A tool for fast downloads.',conflict_handler="resolve")
